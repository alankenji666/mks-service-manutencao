...
function Tracking() {
  ...
  useEffect(() => {
    ...
    axios.get(`https://mks-service-manutencao.onrender.com/api/tracking/${token}`)
      ...
      .catch(err => {
        console.error(err);
        setError('Link de rastreio inválido ou não exite.');
      });

    socket.on('data_updated', (newData) => {
      // Re-filter for active token if incoming full data is emitted
      const pedidoBase = newData.pedidos.find((p: any) => p.TokenAcesso === token);
      if(pedidoBase) {
        const pedidoId = pedidoBase.PedidoID;
        setData({
          pedido: pedidoBase,
          metricas: newData.metricas.filter((m: any) => m.PedidoID === pedidoId),
          progresso: newData.progresso.filter((p: any) => p.PedidoID === pedidoId),
          ocorrencias: newData.ocorrencias.filter((o: any) => o.PedidoID === pedidoId),
        });
      }
    });
  ...
  if (error) return <div style={{padding: 40, fontFamily: 'Inter', textAlign: 'center', color: 'red'}}>{error}</div>;
  ...
  const labels = progresso.filter((p: any) => p.Tipo === 'Previsto').map((p: any) => p.Data);
  const chartData = {
    labels,
    datasets: [
      {
        ...
        borderWidth: 2,
        fill: false,
        data: progresso.filter((p: any) => p.Tipo === 'Previsto').map((p: any) => parseInt(p.Quantidade)),
      },
      {
        type: 'bar' as const,
        label: 'Realizado',
        backgroundColor: '#a8e6cf',
        data: progresso.filter((p: any) => p.Tipo === 'Realizado').map((p: any) => parseInt(p.Quantidade)),
      },
    ],
  };
          ...
          <div className="visao-geral">
            ...
            <div className="visao-chart">
              <div style={{textAlign: 'center'}}>
                ...
                <div style={{marginTop: '-25px', fontSize: '2rem', fontWeight: 800, color: 'var(--mks-green)', position: 'relative', zIndex: 10}}>{(parseFloat(pedido.Percentual || 0) * 100).toFixed(0)}%</div>
